...
impl Editor {
    ...
    pub fn handle_input(&mut self, event: KeyEvent) -> io::Result<EditorEvent> {
        match event.code {
            ...
            KeyCode::Up => {
                ...
                if self.location.row >= self.size.rows - command_offset {
    ...
    pub fn status_bar(&mut self) -> io::Result<()> {
        ...
        if self.mode == EditorMode::NORMAL { offset = 8; }
        self.output
            .queue(cursor::MoveTo(self.size.cols - offset, self.size.rows - command_offset))?;
        print!("");
        ...
        print!(" {:02}:{:02} {}", self.location.col - 5, self.location.row + 1, mode_text);
